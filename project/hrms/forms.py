...
class EmployeeForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = (
            ...
            'language', 'thumb')
        widgets = {
            'salary': forms.TextInput(attrs={'class': 'form-control', 'placeholder': '50000'}),
            'bank': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Meezan Bank'}),
            'accnum': forms.TextInput(attrs={'class': 'form-control', 'placeholder': '0218-365788675677'})
        }
...
class AttendanceForm(forms.ModelForm):
    ...
    staff = forms.ModelChoiceField(
        Employee.objects.filter(~Q(attendance__status=None) and ~Q(attendance__date=timezone.localdate(value=None))),
...
class RecruitmentForm(forms.ModelForm):
    department = forms.ModelChoiceField(Department.objects.all(), required=True, empty_label='Select a department',
                                        widget=forms.Select(attrs={'class': 'form-control'}))
    upload_your_cv = forms.FileField(widget=forms.ClearableFileInput(attrs={'class': 'form-control'}))

    class Meta:
        model = Recruitment
        fields = ('first_name', 'last_name', 'phone', 'email', 'department', 'position', 'upload_your_cv')
        widgets = {
